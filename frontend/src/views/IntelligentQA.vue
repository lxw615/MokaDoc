    ...
    <div class="qa-container">
      ...
      <div class="qa-middle">
        ...
        <div class="chat-container" ref="chatContainer">
          <div 
            ...
            :key="index"
            class="chat-message"
            :class="[`${message.type}-message`, { 'has-references': message.type === 'ai' && message.id }]"
      ...
      <div class="qa-right">
        ...
        <div class="reference-content">
          ...
          >
            <h4>[{{ ref.sourceIndex || index + 1 }}] {{ ref.source }} <span v-if="ref.pageNum">第 {{ ref.pageNum }} 页</span></h4>
            <p>{{ ref.content }}</p>
          </div>
        </div>
      </div>
...
import { listSessions, listChat, createSession, updateSession, deleteSession, listReferences } from '@/api/liaotianguanli'
...
const applyRouteSelectedDocument = () => {
  const queryDocId = Number(route.query.docId)
  if (!Number.isFinite(queryDocId) || queryDocId <= 0) {
    return
  }
  if (documents.value.some(doc => doc.id === queryDocId)) {
    selectedDocs.value = [queryDocId]
  }
}

const applyRouteEntityQuestion = () => {
  const entity = typeof route.query.entity === 'string' ? route.query.entity.trim() : ''
  if (entity && !userInput.value.trim()) {
    userInput.value = `请结合知识图谱说明「${entity}」的相关内容`
  }
}
...
// 引用数据：点击 AI 消息或回答完成后从后端加载
const references = ref<Reference[]>([])
const activeReferenceMessageId = ref<number | null>(null)
...
const clearReferences = () => {
  activeReferenceMessageId.value = null
  references.value = []
}

const getBackendSessionId = (session: Session) => session.backendSessionId || session.id

const normalizeReferenceContent = (content?: string) => {
  const raw = content || ''
  const match = raw.match(/^【引用编号：(\d+)】\s*/)
  return {
    sourceIndex: match ? Number(match[1]) : undefined,
    content: match ? raw.replace(/^【引用编号：\d+】\s*/, '') : raw,
  }
}

// 加载某条 AI 回答的引用溯源
const loadReferences = async (messageId?: number, retryCount = 0) => {
  if (!messageId) {
    clearReferences()
    return
  }
  activeReferenceMessageId.value = messageId
  try {
    const res = await listReferences({ messageId })
    if (res.data.code === 0 && res.data.data) {
      const mappedReferences = res.data.data.map((ref: any, index: number) => {
        const normalized = normalizeReferenceContent(ref.content)
        return {
          source: getDocumentName(Number(ref.documentId)) || `文档ID：${ref.documentId}`,
          content: normalized.content,
          pageNum: ref.pageNum || 0,
          sourceIndex: normalized.sourceIndex || index + 1,
        }
      })
      if (activeReferenceMessageId.value !== messageId) {
        return
      }
      references.value = mappedReferences

      // complete 事件可能比引用查询可见稍早到达，短暂重试避免右侧一直空白。
      if (mappedReferences.length === 0 && retryCount < 5) {
        window.setTimeout(() => loadReferences(messageId, retryCount + 1), 500)
      }
    }
  } catch (error) {
    console.error('加载引用溯源失败:', error)
    if (retryCount < 3) {
      window.setTimeout(() => loadReferences(messageId, retryCount + 1), 500)
    }
  }
...
const handleDeleteSession = (session: Session) => {
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        const res = await deleteSession({ sessionId: getBackendSessionId(session) })
        if (res.data.code === 0) {
          // 从列表中移除
...
const loadSessionMessages = async (sessionId: number) => {
  ...
  if (!session || session.loaded || !session.backendSessionId) {
        ...
        onMessage: (data) => {
          ...
          try {
            ...
            if (parsed.type === 'error') {
              ...
              const errorMessage = parsed.message || 'AI 服务暂时不可用，请稍后重试'
            ...
            if (parsed.type === 'complete') {
              if (parsed.messageId) {
                ...
                loadReferences(parsed.messageId)
              }
              return
            }
